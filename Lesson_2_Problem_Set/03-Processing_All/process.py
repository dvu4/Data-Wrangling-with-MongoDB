...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        dataTDRight = soup.find("table", class_= "dataTDRight").find_all("tr", class_= "dataTDRight")
        #print dataTDRight
        #print dataTDRight[0]
        #print dataTDRight[1]
        #print dataTDRight[2]
        #print dataTDRight[3]
        #print len(dataTDRight)
            
        for row in dataTDRight:
            col = row.find_all("td")
            #print "Year, Month, Domestic, International"
            #print col[0].text, col[1].text,col[2].text, col[3].text
            if col[1].text != "TOTAL":
                info["year"] = int(col[0].text)
                info["month"] = int(col[1].text)
                info["flights"] = {"domestic": int(col[2].text.replace(',', '')),
                                 "international": int(col[3].text.replace(',', ''))}
                data.append(info)
    print data
